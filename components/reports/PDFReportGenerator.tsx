...
export const PDFReportGenerator: React.FC<PDFReportGeneratorProps> = ({ report, branding, organizationName }) => {
    ...
    const scoreColor = report.overallScore >= 70 ? '#059669' : report.overallScore >= 50 ? '#d97706' : '#dc2626';
    const totalRecs = report.pages.reduce((acc, p) => acc + p.recommendations.length, 0);
    const highImpact = report.pages.flatMap(p => p.recommendations).filter(r => r.impact === 'HIGH').length;
    ...
    return (
        <div className="bg-white text-slate-900 w-full max-w-[210mm] mx-auto p-8 print:p-0">
            ...
            <div className="min-h-[297mm] relative flex flex-col">
                <div
                    className="flex-1 flex flex-col items-center justify-center text-center rounded-2xl p-12 relative overflow-hidden"
                    style={{ background: `linear-gradient(135deg, ${branding?.primary_color || '#7c3aed'} 0%, #4f46e5 100%)` }}
                >
                    {branding?.logo_url && (
                        <img src={branding.logo_url} alt="Logo" className="h-16 mb-8 object-contain" />
                    )}
                    <h1 className="text-4xl font-black text-white mb-3">AI Visibility Report</h1>
                    <p className="text-white/80 text-lg mb-8">{organizationName || 'Your Organization'}</p>

                    <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-8 inline-block mb-8">
                        <div className="text-6xl font-black text-white mb-1" style={{ textShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
                            {report.overallScore}
                        </div>
                        <div className="text-white/70 text-sm font-semibold uppercase tracking-widest">Overall Score</div>
                    </div>

                    <div className="grid grid-cols-3 gap-6 w-full max-w-sm">
                        <div className="text-center">
                            <div className="text-2xl font-bold text-white">{report.platformScores.length}</div>
                            <div className="text-white/60 text-xs">Platforms</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-white">{report.pages.length}</div>
                            <div className="text-white/60 text-xs">Pages Audited</div>
                        </div>
                        <div className="text-center">
                ...
                <div className="mt-auto pt-6 flex justify-between text-xs text-slate-400">
                    <span>Generated {reportDate} by Cognition AI</span>
                    <span>{highImpact} high-impact action items</span>
                </div>
            </div>
            ...
            <div className="min-h-[297mm] relative flex flex-col">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="bg-slate-50 p-6 rounded-xl border border-slate-100">
                                ...
                                <div className="flex items-end gap-2 mb-1">
                                    <span className="text-3xl font-black text-slate-900">{report.brandConsistencyScore}%</span>
                                </div>
                ...
                <div className="mt-auto pt-8 border-t border-slate-200 flex justify-between text-xs text-slate-400">
                    <span>Generated {reportDate} by Cognition AI</span>
                    <span>Page 2 of {Math.ceil((totalRecs / 4) + 2)}</span>
                </div>
            </div>
            ...
            <div className="min-h-[297mm] relative flex flex-col pt-8">
                ...
                <div className="mt-auto pt-8 border-t border-slate-200 flex justify-between text-xs text-slate-400">
                    <span>Generated {reportDate} by Cognition AI &middot; {organizationName || ''}</span>
