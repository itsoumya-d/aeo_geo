...
import { Report, SocialAnalysis, SocialPlatformResult } from '../../types';
import {
...
const PLATFORM_COLORS: Record<string, string> = {
    'Twitter/X': 'text-sky-500 bg-sky-50 border-sky-200',
    ...
    return (
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
            ...
            <text
                ...
                className="rotate-90"
                style={{ transform: `rotate(90deg) translate(0, 0)`, transformOrigin: `${size / 2}px ${size / 2}px`, fontSize: '12px', fontWeight: 700, fill: color }}
            >
                {score}
            </text>
        </svg>
        ...
        <div className={`bg-white border rounded-2xl p-5 ${platform.present ? 'border-blue-100' : 'border-slate-200 opacity-60'}`}>
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-semibold text-slate-800 text-sm">{platform.platform}</div>
                        {platform.present ? (
                ...
                {platform.present && (
                    <div className="flex items-center gap-3">
                        <div className="text-center">
                            <div className="text-[10px] text-slate-500 font-medium mb-0.5">Activity</div>
                            <ScoreRing score={platform.activityScore} size={44} />
                        </div>
                        <div className="text-center">
                            <div className="text-[10px] text-slate-500 font-medium mb-0.5">Consistency</div>
            ...
            {platform.bio && (
                <p className="text-xs text-slate-500 italic mb-3 border-l-2 border-blue-200 pl-3">"{platform.bio}"</p>
            )}
    ...
    if (!social) {
        return (
            ...
            >
                ...
                <h3 className="text-lg font-semibold text-slate-700 mb-2">Social Media Analysis Not Available</h3>
                <p className="text-sm text-slate-500 max-w-md mx-auto">
                    Add your social media profiles during onboarding or in the Input step to enable social presence scoring.
                </p>
            </motion.div>
        );
    }
        ...
        >
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-blue-100">
                <div>
                    ...
                    <p className="text-slate-500 mt-2 text-sm max-w-xl">
                        AI visibility is 15% influenced by social brand consistency. Mismatched profiles reduce citation likelihood.
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                    ...
                    const color = value < 40 ? 'text-red-600 bg-red-50 border-red-100' : value < 70 ? 'text-amber-600 bg-amber-50 border-amber-100' : 'text-green-600 bg-green-50 border-green-100';
                    return (
            ...
            {/* Platform grid */}
            <div>
                <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wider mb-4">Platform Breakdown</h3>
            ...
            {social.recommendations && social.recommendations.length > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-sm font-bold text-blue-800">Recommendations</h3>
                    </div>
                    <div className="space-y-2">
