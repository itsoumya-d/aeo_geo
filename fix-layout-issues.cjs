...
// 2. Widen container to prevent white gap overflow
content = content.replace(
...
// 3. Revert hacky negative margins on table
content = content.replace(
...
// 4. Restore the Email Support button fix that git restore destroyed
content = content.replace(
...
console.log('Fixed layout issues successfully.');
